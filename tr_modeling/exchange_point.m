function [model, succeeded, pt_i] = exchange_point(model, new_point, new_fvalues, relative_pivot_threshold)
    ...
    if last_p >= 2
        ...
        if abs(new_pivot_val) > pivot_threshold
            ...
            % Re-orthogonalize
            pivot_polynomials(max_poly_i) = orthogonalize_to_other_polynomials(pivot_polynomials, max_poly_i, points_shifted, last_p);
